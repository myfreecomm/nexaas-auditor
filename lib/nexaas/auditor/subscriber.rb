  ...
  module Auditor
    class Subscriber
      ...
      def call(name, start, finish, event_id, payload)
        ...
        send(method_name, name, start, finish, event_id, payload) if respond_to?(method_name)
      end

      private

      def event_method_name(_name)
      ...
      def self.validate_subclasses!
        raise "no subclasses of #{self} found!" if subclasses.empty?
      end
    end
  end
end
